...
// Reserve address 0x0000 of XDATA space. On reset or waking from sleep mode, this
// location may be overwritten with random data (see ref manual).
SI_LOCATED_VARIABLE_NO_INIT(pwr_reserved, uint8_t, SI_SEG_XDATA, 0x0000);

// Reads and clears all PMU0 wake-up flags.
uint8_t PWR_readAndClearWakeFlags(void)
{
  uint8_t save_ie, snapshot_pmu0cf, snapshot_pmu0fl;
  SAVE_SFRPAGE;
  SFRPAGE = 0;
  ...
  // Combine the snapshots into one bitmask and return it
  snapshot_pmu0cf &= PWR_PMU0CF_WAKE_FLAGS;
  snapshot_pmu0cf |= (snapshot_pmu0fl & PMU0FL_CS0WK__BMASK) ? PWR_CS0_WF : 0;
  return snapshot_pmu0cf;
}
...
void PWR_enterSuspend(uint8_t flags)
{
  ...
  PMU0FL = (flags & PWR_CS0_WF) ? PMU0FL_CS0WK__SET : 0;
