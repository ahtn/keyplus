 ...
 * @{
 *
 * @brief Power mode driver for the EFM8BB3.
 *
 * # Introduction #
 *
 * This module provides an API for entering each of the low power modes of
 * the EFM8BB3. The API reconfigures the system as required to enter each
 ...
 * |-----------|-----------------------------|------|-------|-------|------|
 * |Default    |                             |  120 |     0 |     0 |    0 |
 * |Stop       |EFM8PDL_PWR_USE_STOP         |   +0 |     0 |     0 |    0 | 
 ...
 * @{
 *
 * The following macros define the wake-up bits used to read the power
 * management unit status (PSTAT0). These values are also used to form 
 * the return value for PWR_readAndClearWakeFlags().
 *****************************************************************************/

/// Wake-up from a Comparator 0 rising edge.
#define PWR_CPT0_WF PSTAT0_CPT0WK__BMASK 

/// Wake-up from a Port Match event.
#define PWR_PMAT_WF PSTAT0_PMATWK__BMASK

/// Wake-up from a Timer 4 event.
#define PWR_TMR4_WF PSTAT0_TMR4WK__BMASK 

/// Wake-up from an I2C0 Slave activity.
#define PWR_I2C0_WF PSTAT0_I2C0WK__BMASK

/// Wake-up from a SPI0 Slave activity.                          
#define PWR_SPI0_WF PSTAT0_SPI0WK__BMASK

/// Wake-up from a Configurable Logic Wake event.
#define PWR_CL0_WF PSTAT0_CL0WK__BMASK

/** @} (end addtogroup pwr_wakeflags) */

/**************************************************************************//**
 * Reads then clears all PSTAT0 wake-up flags.
 *
 ...
 *
 * PSTAT0 register captures system events that wake-up the device 
 * from either the Suspend or Sleep modes. This function reads and clears this 
 * register. The wake-up flags are sticky and must be cleared by 
 * firmware.
 *
 * ~~~~~.c
 * uint8_t wflags;
 ...
 * ~~~~~
 *
 * @note The application should always call this function after waking up 
 * since the wake-up flags are sticky. Wake-up flags may not be correct if 
 * the application re-enters Suspend or Sleep mode without clearing the flags
 * 
 *****************************************************************************/
