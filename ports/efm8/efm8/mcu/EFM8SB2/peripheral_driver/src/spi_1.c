...
// not called.
// ----------------------------------------------------------------------------
static void SPI1_internalInit(void)
{
  // figure out if we are master mode, and using 4-wire or not
  modeIsMaster = SPI1CFG & SPI1CFG_MSTEN__MASTER_ENABLED;
  useNss = SPI1CN0 & SPI1CN0_NSSMD__FMASK;
...
// ----------------------------------------------------------------------------
void SPI1_clearIntFlags(uint8_t flags)
{
  DECL_PAGE;
  SET_PAGE(SPI_SFR_PAGE);
  SPI1CN0 = (SPI1CN0 & (~flags | 0x0F));
  RESTORE_PAGE();
}
#endif
...
// ----------------------------------------------------------------------------
bool SPI1_writeByte(uint8_t value)
{
  bool ret = false;
  DECL_PAGE;
...
// ----------------------------------------------------------------------------
void SPI1_pollWriteByte(uint8_t value)
{
  DECL_PAGE;
...
// ----------------------------------------------------------------------------
uint8_t
SPI1_pollTransfer(SI_VARIABLE_SEGMENT_POINTER(pTxBuffer, uint8_t,
                                              EFM8PDL_SPI1_TX_SEGTYPE),
                  SI_VARIABLE_SEGMENT_POINTER(pRxBuffer, uint8_t,
                                              EFM8PDL_SPI1_RX_SEGTYPE),
                  SPI1_TransferDirection_t dir,
                  uint8_t xferCount)
{
  ...
  while (SPI1CFG & SPI1CFG_SPIBSY__BMASK)
  {}
  ...
  while (xferCount)
  {
    ...
    if (SPI1CN0_TXBMT && txCount)
    {
      // If user provided a write buffer then use that
      if (dir & SPI1_TRANSFER_TX)
      {
        SPI1DAT = *pTxBuffer;
        ++pTxBuffer;
      }
    ...
    if (!modeIsMaster && useNss)
    {
      ...
      if (!checkNss)
      {
        if ((SPI1CFG & SPI1CFG_SLVSEL__BMASK))
        {
          checkNss = true;
        }
      }
...
bool SPI1_isBusy(void)
{
  ...
  {
    if (SPI1CFG & SPI1CFG_SPIBSY__BMASK)
    {
      ret = true;
    }
  }
...
// ----------------------------------------------------------------------------
bool SPI1_isSlaveSelected(void)
{
  bool ret;
  DECL_PAGE;

  SET_PAGE(SPI_SFR_PAGE);
  ret = SPI1CFG & SPI1CFG_SLVSEL__BMASK;
  RESTORE_PAGE();
  return ret;
}
...
// ----------------------------------------------------------------------------
bool SPI1_isShiftEmpty(void)
{
  bool ret;
  DECL_PAGE;

  SET_PAGE(SPI_SFR_PAGE);
  ret = SPI1CFG & SPI1CFG_SRMT__BMASK;
  RESTORE_PAGE();
  return ret;
}
...
// ----------------------------------------------------------------------------
bool SPI1_isRxEmpty(void)
{
  bool ret;
  DECL_PAGE;

  SET_PAGE(SPI_SFR_PAGE);
  ret = SPI1CFG & SPI1CFG_RXBMT__BMASK;
  RESTORE_PAGE();
  return ret;
}
#endif
...
// ----------------------------------------------------------------------------
void SPI1_init(SPI1_ClockMode_t clockMode, bool isMasterMode, bool is4wire)
{
  DECL_PAGE;
  ...
  SPI1CFG = clockMode | (isMasterMode ? SPI1CFG_MSTEN__MASTER_ENABLED
                                      : SPI1CFG_MSTEN__MASTER_DISABLED);
  ...
  {
    SPI1CN0_NSSMD0 = 1;
    SPI1CN0_NSSMD1 = isMasterMode ? 1 : 0;
  }
  else
  {
    SPI1CN0_NSSMD0 = 0;
...
// ----------------------------------------------------------------------------
void SPI1_setClockDivider(uint16_t divider)
{
  DECL_PAGE;
...
static SI_VARIABLE_SEGMENT_POINTER(pTxBuf, uint8_t, EFM8PDL_SPI1_TX_SEGTYPE) = NULL;
static SI_VARIABLE_SEGMENT_POINTER(pRxBuf, uint8_t, EFM8PDL_SPI1_RX_SEGTYPE) = NULL;

// ----------------------------------------------------------------------------
// Set up an interrupt driven SPI transfer.
// ----------------------------------------------------------------------------
void
SPI1_transfer(SI_VARIABLE_SEGMENT_POINTER(pTxBuffer, uint8_t,
                                          EFM8PDL_SPI1_TX_SEGTYPE),
              SI_VARIABLE_SEGMENT_POINTER(pRxBuffer, uint8_t,
                                          EFM8PDL_SPI1_RX_SEGTYPE),
              SPI1_TransferDirection_t dir,
              uint8_t xferCount)
{
  ...
  pTxBuf = pTxBuffer;
  pRxBuf = pRxBuffer;
  useRx = dir & SPI1_TRANSFER_RX;
  useTx = dir & SPI1_TRANSFER_TX;
...
#if EFM8PDL_SPI1_USE_PIPELINE == 0
  // Write the first byte to get the transfer started
  if (SPI1CN0_TXBMT && txCountRemaining)
  {
  ...
  while (SPI1CN0_TXBMT && txCountRemaining)
  {
    // If user provided tx data, then use that
    if (useTx)
    {
...
// ----------------------------------------------------------------------------
uint8_t SPI1_bytesRemaining(void)
{
...
SI_INTERRUPT(SPI1_ISR, SPI1_IRQn)
{
  ...
  intFlags = SPI1CN0 & 0xF0;
  SPI1CN0 = (SPI1CN0 & (~intFlags | 0x0F));
  ...
  if (intFlags & SPI1_INT_IF)
  {
    ...
    if (txCountRemaining && SPI1CN0_TXBMT)
    {
  ...
  if (intFlags & (SPI1_MODF_IF | SPI1_RXOVR_IF))
  {
    ...
    SPI1_transferErrorCb(intFlags & (SPI1_MODF_IF | SPI1_RXOVR_IF));
  }
