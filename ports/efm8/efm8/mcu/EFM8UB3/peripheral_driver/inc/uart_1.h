 ...
 *     UART1_writeBuffer(bufferA, curBuffer - bufferA);
 *     curBuffer = bufferB;
 *     targetA = false;
 *   }
 *   else
 ...
 * at least 310 prescaled clocks per bit in the sync word (0x55) that was received. 
...
typedef enum
{
  UART1_RX_ENABLE  = SCON1_REN__RECEIVE_ENABLED,   //!< UART Receive Enabled.
  UART1_RX_DISABLE = SCON1_REN__RECEIVE_DISABLED,  //!< UART Receive Disabled.
} UART1_RxEnable_t;
...
typedef enum
{
  ...
  UART1_TXTHRSH_ONE   = UART1FCN0_TXTH__ONE,   //!< UART Transmit Fifo Threshold 1.
  UART1_TXTHRSH_TWO   = UART1FCN0_TXTH__TWO,   //!< UART Transmit Fifo Threshold 2.
  UART1_TXTHRSH_THREE = UART1FCN0_TXTH__THREE, //!< UART Transmit Fifo Threshold 3.
...
typedef enum
{
  ...
  UART1_RXTHRSH_ONE   = UART1FCN0_RXTH__ONE,   //!< UART Receive Fifo Threshold 1.
  UART1_RXTHRSH_TWO   = UART1FCN0_RXTH__TWO,   //!< UART Receive Fifo Threshold 2.
  UART1_RXTHRSH_THREE = UART1FCN0_RXTH__THREE, //!< UART Receive Fifo Threshold 3.
 ...
 *
 * @param txth:
 * Count of bytes in TX Fifo to trigger a request (0-3).
 * @param txint:
 ...
 * can be a value from 0-3. For the transmit threshold, the request is triggered 
 ...
 *
 * @param rxth:
 * Count of bytes in RX Fifo to trigger a request (0-3).
 * @param rxto:
 ...
 * "UART1_getFifoIntFlags()". The threshold can be a value from 0-3. For the 
