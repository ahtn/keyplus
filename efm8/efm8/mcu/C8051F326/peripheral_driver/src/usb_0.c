...
void USB_SetNormalClock(void)
{
  uint16_t i;

  //Ensure a full speed HFOSC is enabled
  OSCICN |= (OSCICN_IOSCEN__ENABLED | OSCICN_IFCN__HFOSC_DIV_1);

  //If multiplier is not ready we need to start it
  if (!(CLKMUL & CLKMUL_MULRDY__BMASK))
  {
    //Startup multiplier
    CLKMUL = 0x00;
