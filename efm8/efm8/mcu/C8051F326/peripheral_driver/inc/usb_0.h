...
#else
#define USB_EnableInts() do {EIE1 |= EIE1_EUSB0__ENABLED;} while (0)
#endif
...
#else
#define USB_DisableInts() do {EIE1 &= ~EIE1_EUSB0__ENABLED;} while (0)
#endif
...
#else
#define USB_GetIntsEnabled() (EIE1 & EIE1_EUSB0__ENABLED)
#endif
...
#else
#define USB_IsVbusOn() ((bool)(REG0CN & REG0CN_VBSTAT__SET))
#endif
...
#define USB_SetClockIntOscDiv8() \
  ...
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__HFOSC_DIV_8; \
  } while (0)
#endif
...
#define USB_SetClockExtOscDiv2() \
  ...
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__EXTOSC_DIV_2; \
  } while (0)
#endif
...
#define USB_SetClockExtOscDiv3() \
  ...
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__EXTOSC_DIV_3; \
  } while (0)
#endif
...
#define USB_SetClockExtOscDiv4() \
  ...
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__EXTOSC_DIV_4; \
  } while (0)
#endif
...
#define USB_SetClockLfo() \
  ...
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__LFOSC; \
  } while (0)
#endif

/***************************************************************************//**
 * @brief       Selects the normal setting for the USB clock
 * @details     This function will lock the PLL if necessary.
 ******************************************************************************/
...
#else
#define USB_SetSuspendClock() \
  do \
  { \
    CLKSEL &= ~CLKSEL_USBCLK__FMASK; \
    CLKSEL |= CLKSEL_USBCLK__HFOSC_DIV_2; \
  } while (0)
#endif

/***************************************************************************//**
 * @brief       Suspends REG0 and REG1
 * @note        This function is implemented as a macro.
 ******************************************************************************/
#ifdef IS_DOXYGEN
...
#define USB_SuspendRegulator() \
  do \
  { \
    REG0CN |= REG0CN_REG0MD__LOW_POWER; \
  } while (0)
#endif
...
#else
#define USB_IsRegulatorEnabled() (!(REG0CN & REG0CN_REG0DIS__DISABLED))
#endif
...
#else
#define USB_IsPrefetchEnabled() false
#endif
...
// large for that endpoint
      ...
      #error EP1IN packet size too large. Packet size must be 64 bytes or less.
    #endif          // #if SLAB_USB_EP1IN_MAX_PACKET_SIZE > 64
    #if SLAB_USB_EP1IN_TRANSFER_TYPE == USB_EPTYPE_ISOC
      #error Isochronous transfers are not supported on this device.
    ...
    #if SLAB_USB_EP1OUT_MAX_PACKET_SIZE > 128
      #error EP1OUT packet size too large. Packet size must be 128 bytes or less.
  ...
  #if SLAB_USB_EP2IN_USED == 1
    #error EP2 does not exist on this part
  #endif
  ...
  #if SLAB_USB_EP2IN_USED == 1
    #error EP3 does not exist on this part
  #endif
