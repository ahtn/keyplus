...
# define SI_SBIT(name, addr, bit)  __sbit  __at(addr+bit)                    name
# define SI_SFR(name, addr)        __sfr   __at(addr)                        name
# define SI_SFRX(name, addr)       __xdata volatile unsigned char __at(addr) name
# define SI_SFR16(name, addr)      __sfr16 __at(((addr+1U)<<8) | addr)       name
# define SI_SFR16E(name, fulladdr) __sfr16 __at(fulladdr)                    name
# define SI_SFR32(name, addr)      __sfr32 __at(((addr+3UL)<<24) | ((addr+2UL)<<16) | ((addr+1UL)<<8) | addr) name
# define SI_SFR32E(name, fulladdr) __sfr32 __at(fulladdr)                    name
...
# define SI_INTERRUPT_USING(name, vector, regnum) void name (void) interrupt (vector) using (regnum)
# define SI_INTERRUPT_PROTO(name, vector) void name (void) interrupt (vector)
# define SI_INTERRUPT_PROTO_USING(name, vector, regnum) void name (void) interrupt (vector) using (regnum)

# define SI_FUNCTION_USING(name, return_value, parameter, regnum) return_value name (parameter) using (regnum)
# define SI_FUNCTION_PROTO_USING(name, return_value, parameter, regnum) return_value name (parameter) using (regnum)
// Note: Parameter must be either 'void' or include a variable type and name. (Ex: char temp_variable)
...
# define SI_SEGMENT_VARIABLE(name, vartype, locsegment) locsegment vartype name
# define SI_VARIABLE_SEGMENT_POINTER(name, vartype, targsegment) targsegment vartype * name
# define SI_SEGMENT_VARIABLE_SEGMENT_POINTER(name, vartype, targsegment, locsegment) targsegment vartype * locsegment name
# define SI_SEGMENT_POINTER(name, vartype, locsegment) vartype * locsegment name
# define SI_LOCATED_VARIABLE(name, vartype, locsegment, addr, init) locsegment at (addr) vartype name = init
# define SI_LOCATED_VARIABLE_NO_INIT(name, vartype, locsegment, addr) locsegment at (addr) vartype name
...
# define SI_SEGMENT_POINTER(name, vartype, locsegment) vartype * locsegment name
# define SI_LOCATED_VARIABLE(name, vartype, locsegment, addr, init) at addr locsegment vartype name
# define SI_LOCATED_VARIABLE_NO_INIT(name, vartype, locsegment, addr) at addr locsegment vartype name
